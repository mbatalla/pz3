	...
	protected function generateCMSCategoryData()
	{
		...
		if ($cms_categories)
		{
			foreach ($cms_categories as $cms_category)
			{
				...
				{
					$cms_cat = new CMSCategory($cc['infos']['id_cms_category'], $id_lang);

					foreach ($this->cms_category_fields as $field => $array)
						$line[$field] = property_exists('CMSCategory', $field) && !is_array($cms_cat->$field) && !Tools::isEmpty($cms_cat->$field) ? $cms_cat->$field : '';
