...
class AdminSampleDataInstallImportController extends ModuleAdminController {
	...
	public function postProcess()
	{
		if (Tools::getValue('action') == 'installData')
		{
			...
			if ($all == 'true') {
				...
				$this->importConfigurations(true, true, true);
			}

			if ($all == 'false' && $modules == 'true') {
				$this->clearDb();
				$this->importConfigurations(false, true);
				$this->importSQLdata();
			}

			if ($all == 'false' && $logos == 'true') {
				$this->importConfigurations(true);
			}

			if ($all == 'false' && $content == 'true') {
				$this->importConfigurations(false, false, true);
				$this->importAlias();
				$this->importAttachments();
				$this->importAttributeGroups();
				$this->importAttributes();
	...
	**/
	...
	protected function importConfigurations($logos = false, $modules = false, $content = false)
	{
	...
	public function filterFields($class_name, $fields, $line)
	{
		if (count($fields) != count($line)) {
            $this->clearTemporaryFolder();
            die(Tools::jsonEncode(array('status' => 'error', 'responseText' => sprintf($this->l('Look like your %s file structure is incorrect'), $class_name))));
        }
