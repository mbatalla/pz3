...
class TmSocialFeeds extends Module
{
    public function __construct()
    {
        ...
        $this->module_key = '1b8330574126dba0b02e91dcf9560a5b';
        parent::__construct();
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit'.$this->name)) {
            ...
            } elseif ($tmfi_display && (!$tmfi_user_id || empty($tmfi_user_id)) && $tmfi_type == 'user') {
                $output .= $this->displayError($this->l('Invalid Instagram Client Id value'));
            } elseif ($tmfi_display && ($tmfi_user_id || empty($tmfi_user_id))
                    && $tmfi_type == 'tagged' && (!$tmfi_tag || empty($tmfi_tag))) {
                $output .= $this->displayError($this->l('"Instagram Tag" is required for "tagged" mode.'));
//            } elseif ($tmfi_display && ($tmfi_id || empty($tmfi_id))
//                    && $tmfi_type == 'user'
//                    && ((!$tmfi_user_id || empty($tmfi_user_id)) || (!$tmfi_access || empty($tmfi_access)))) {
//                $output .= $this->displayError(
//                    $this->l('"Instagram User ID" and "Instagram Access Token" are required for "user" mode.')
//                );
            } else {
    ...
    protected function addItem(
        ...
        if (!Db::getInstance()->Execute(
            ...
            (`id_shop`,`item_type`, `item_order`, `item_theme`, `hook`, `item_width`, `item_height`,
            `item_limit`, `item_header`, `item_footer`, `item_border`, `item_replies`, `item_scroll`,
    ...
    protected function updateItem(
        ...
        if (!Db::getInstance()->execute(
            'UPDATE `'._DB_PREFIX_.'tmsocialfeed` SET 
              ...
              active = '.(int)$active.'
            WHERE item_type = "'.pSQL($item_type).'"
...
//                    'desc'        => $this->l('A valid oAuth token. Required to use "user" mode.'),
    ...
    public function displayNewContainerForm()
    {
        ...
        $this->context->smarty->assign('htmlitems', array(
            ...
            'postAction' => 'index.php?tab=AdminModules&configure='.$this->name.'
                                                        &token='.Tools::getAdminTokenLite('AdminModules').'
                                                        &tab_module=other
    ...
    protected function transformLCode($code)
    {
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $this->context->controller->addCSS($this->_path.'views/css/tmsocialfeeds_admin.css');
        $this->context->controller->addJquery();
        $this->context->controller->addJS($this->_path.'views/js/tmsocialfeeds_admin.js');
    }

    public function hookDisplayHeader()
    {
        $this->context->controller->addJS($this->_path.'views/js/tmsocialfeeds.js');
        ...
        $this->context->controller->addCSS($this->_path.'views/css/tmsocialfeeds.css');
    }

    public function getInstagramContent()
    {
        if (Configuration::get('TMINSTAGRAM_TYPE') == 'tagged' && Configuration::get('TMINSTAGRAM_TAG') != '') {
            $insta_params = json_decode(file_get_contents('https://www.instagram.com/explore/tags/' . Configuration::get('TMINSTAGRAM_TAG') . '/?__a=1'), true);
            return $insta_params['tag'];
        } else if (Configuration::get('TMINSTAGRAM_TYPE') == 'user' && Configuration::get('TMINSTAGRAM_USERID') != '') {
            $insta_params = json_decode(file_get_contents('https://www.instagram.com/' . Configuration::get('TMINSTAGRAM_USERID') . '/?__a=1'), true);
            return $insta_params['user'];
        }
        return '';
    }
    ...
    {
        $instagram_content = '';
        if (Configuration::get('TMINSTAGRAM_DISPLAY')) {
            $instagram_content = $this->getInstagramContent();
        }

        $this->context->smarty->assign('l_code', $this->transformLCode($this->context->language->language_code));
        $this->context->smarty->assign('hook_content', $this->getHookItems('home'));
        $this->context->smarty->assign('hook_name', 'home');
        $this->context->smarty->assign('instagram_param', $instagram_content);
