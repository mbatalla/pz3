...
{foreach from=$items item=item key=item_key name=item_info}
    {capture name=some_content assign=popText}
        <section class="rd-parallax rd-parallax-{$smarty.foreach.item_info.iteration|escape:'htmlall':'UTF-8'}" style="position: relative; background-image: url('{$base_url|escape:'htmlall':'UTF-8'}{$item.image|escape:'quotes':'UTF-8'}'); background-size: cover; overflow:hidden;">
            {if $item.full_width == 1}<div class="container">{/if}<div class="parallax-main-layout"></div>{if $item.full_width == 1}</div>{/if}
        </section>
    {/capture}
    <script>
