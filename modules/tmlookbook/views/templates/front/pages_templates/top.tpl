...
<div class="row tm-lookbook-block">
  <div class="col-sm-12">
        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="hotSpotWrap hotSpotWrap_{$tab.id_tab|escape:'htmlall':'UTF-8'}_{$smarty.foreach.tab.iteration|escape:'htmlall':'UTF-8'}">
              <img src="{$base_dir|escape:'htmlall':'UTF-8'}{$tab.image|escape:'htmlall':'UTF-8'}" style="max-width:100%" alt="">
            ...
            <div class="caption hidden">
              <h3>{$tab.name|escape:'htmlall':'UTF-8'}</h3>
              <p>{$tab.description}</p>
            </div>
          </div>
    ...
    <ul class="tab-list clearfix" style="text-align: center;">
      {foreach from=$tabs item=tab}
        <li>
          <a href="#id_tab={$tab.id_tab|escape:'htmlall':'UTF-8'}" data-id="{$tab.id_tab|escape:'htmlall':'UTF-8'}">
            <img src="{$tab.image|escape:'htmlall':'UTF-8'}" style="max-width:100%" alt="">
          </a>
        </li>
      {/foreach}
    </ul>
...
<script>
  $(document).ready(function(){
    ...
    if (!id_tab || (typeof tmml_page_name != 'undefined' && tmml_page_name == 'index')) {
      $('.lookbook-tab:first-child').removeClass('hidden');
