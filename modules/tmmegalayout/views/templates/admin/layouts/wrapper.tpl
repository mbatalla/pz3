    ...
    <article class="wrapper-inner clearfix inner">
        {if $preview == false}
            <div class="button-container clearfix">
                <span class="element-name">{l s='Wrapper' mod='tmmegalayout'} <span class="sort-order">{$elem.sort_order|escape:'html':'UTF-8'}</span> <span class="identificator">{if $elem.specific_class}({$elem.specific_class|replace:' ':' | '}){/if}</span></span>
