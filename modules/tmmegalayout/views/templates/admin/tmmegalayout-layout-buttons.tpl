...
{if isset($content.layout)}
    <p {if isset($content.layout)}data-layout-id="{$content.id_layout|escape:'htmlall':'UTF-8'}"{/if} class="tmlist-layout-btns pull-left">
        <a data-layout-id="{$content.id_layout|escape:'htmlall':'UTF-8'}" href="#" class="{if $content.status || $content.partly_use}hidden{/if} layout-btn use-layout"><i class="process-icon-toggle-off"></i> {l s='Use as default' mod='tmmegalayout'}</a>
        <a data-layout-id="{$content.id_layout|escape:'htmlall':'UTF-8'}" href="#" class="{if !$content.status && !$content.partly_use}hidden{/if} layout-btn disable-layout"><i class="process-icon-toggle-on"></i> {l s='Use as default' mod='tmmegalayout'}</a>
    </p>
