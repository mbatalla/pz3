...
<p class="alertMessage alert alert-warning {if Configuration::get(TMMEGALAYOUT_SHOW_MESSAGES) != '1' || Configuration::get(TMMEGALAYOUT_OPTIMIZE) == '1'}hidden{/if}">{l s='Option `optimization` activated. After you complete all actions with presets, click optimize button.' mod='tmmegalayout'} <a id="optionOptimize" class="btn btn-success btn-sm pull-right" href="#">{l s='Optimize' mod='tmmegalayout'}</a></p>
    ...
    {foreach from=$tabs item=tab name=tabs}
        <li id="tab-{$smarty.foreach.tabs.iteration|escape:'htmlall':'UTF-8'}" class="{$tab.id|escape:'htmlall':'UTF-8'} {if $smarty.foreach.tabs.iteration == 1}active{/if}" {if isset($tab.section_name)}data-section="{$tab.section_name|escape:'htmlall':'UTF-8'}"{/if}>
            {if $tab.type != 'sections'}
                <a href="#items-{$smarty.foreach.tabs.iteration|escape:'htmlall':'UTF-8'}" data-toggle="tab" id="{$tab.id|escape:'htmlall':'UTF-8'}" {if isset($tab.hook_name)}data-tab-name="{$tab.hook_name|escape:'htmlall':'UTF-8'}" class="layouts-tab"{/if}>{$tab.tab_name|escape:'htmlall':'UTF-8'}</a>
