    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post">
      ...
      <div class="form_content clearfix">
        <div class="form-group">
          ...
          <input class="is_required validate account_input form-control email" data-validate="isEmail" type="text" name="header-email" id="header-email" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes|escape:'htmlall':'UTF-8'}{/if}"/>
        </div>
        <div class="form-group">
          <label for="header-passwd">{l s='Password' mod='tmheaderaccount'}</label>
            <span>
              <input class="is_required validate account_input form-control password" type="password" data-validate="isPasswd" name="header-passwd" id="header-passwd" value="{if isset($smarty.post.passwd)}{$smarty.post.passwd|stripslashes|escape:'htmlall':'UTF-8'}{/if}" autocomplete="off"/>
            </span>
        </div>
        <p class="submit">
    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" class="std">
      ...
      <div class="account_creation">
        <div class="clearfix">
          ...
          {foreach from=$genders key=k item=gender}
            <div class="radio-inline">
              <label for="id-gender{$gender->id}" class="top">
        ...
        <div class="required form-group">
          <label for="customer-firstname">{l s='First name' mod='tmheaderaccount'} <sup>*</sup></label>
          <input onkeyup="$('#firstname').val(this.value);" type="text" class="is_required validate form-control" data-validate="isName" name="firstname" id="customer-firstname" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}"/>
        </div>
        <div class="required form-group">
          <label for="customer-lastname">{l s='Last name' mod='tmheaderaccount'} <sup>*</sup></label>
          <input onkeyup="$('#lastname').val(this.value);" type="text" class="is_required validate form-control" data-validate="isName" name="lastname" id="customer-lastname" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}"/>
        </div>
        <div class="required form-group">
          <label for="email-create">{l s='Email' mod='tmheaderaccount'} <sup>*</sup></label>
          <input type="email" class="is_required validate form-control" data-validate="isEmail" name="email" id="email-create" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}"/>
        </div>
        <div class="required password form-group">
          <label for="passwd-create">{l s='Password' mod='tmheaderaccount' mod='tmheaderaccount'} <sup>*</sup></label>
            ...
            <div class="col-xs-6">
              <select name="months" class="form-control">
                ...
                {foreach from=$months key=k item=month}
                  <option value="{$k}" {if ($sl_month == $k)} selected="selected"{/if}>{l s=$month mod='tmheaderaccount'}&nbsp;</option>
                {/foreach}
              </select>
            </div>
            <div class="col-xs-3">
        ...
        {if isset($newsletter) && $newsletter}
          <div class="checkbox">
            ...
            <label for="newsletter-tmha">{l s='Sign up for our newsletter!' mod='tmheaderaccount'}</label>
        ...
        {if isset($optin) && $optin}
          <div class="checkbox">
            ...
            <label for="optin-tmha">{l s='Receive special offers from our partners!' mod='tmheaderaccount'}</label>
        ...
        <div class="account_creation">
          ...
          {foreach from=$dlv_all_fields item=field_name}
            {if $field_name eq "company"}
              {if !$b2b_enable}
                <p class="form-group">
                  <label for="company2-tmha">{l s='Company' mod='tmheaderaccount'}{if in_array($field_name, $required_fields)}<sup>*</sup>{/if}
                  </label>
          ...
          {if isset($one_phone_at_least) && $one_phone_at_least}
            ...
            <p class="inline-infos required">** {l s='You must register at least one phone number.' mod='tmheaderaccount'}</p>
          {/if}
          <p class="required form-group">
            <label for="alias-tmha">{l s='Assign an address alias for future reference.' mod='tmheaderaccount'} <sup>*</sup></label>
