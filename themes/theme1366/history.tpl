        ...
        {foreach from=$orders item=order name=myLoop}
          <tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
            <td class="history_link bold">
              ...
              {/if}
              <a class="color-myaccount" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true, NULL, "id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}');" title="{Order::getUniqReferenceOf($order.id_order)}">
            ...
            <td class="history_detail">
              <a class="btn btn-secondary btn-sm icon-right" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true, NULL, "id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}');" title="{l s='Details'}">
                <span>
                  {l s='Details'}
                </span>
              </a>
...
<ul class="footer_links clearfix">
  <li>
    <a class="btn btn-secondary btn-sm icon-left" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='Back to Your Account'}">
      <span>
  ...
  <li>
    <a class="btn btn-secondary btn-sm icon-left" href="{$base_dir}" title="{l s='Home'}">
      <span>
        {l s='Home'}
      </span>
    </a>
