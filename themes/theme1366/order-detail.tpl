{if isset($order)}
  <div class="box box-small clearfix">
    {if isset($reorderingAllowed) && $reorderingAllowed}
      <form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
        ...
        <a href="#" onclick="$(this).closest('form').submit(); return false;" class="btn btn-secondary btn-sm pull-right"><span>{l s='Reorder'}</span></a>
      </form>
    {/if}
    <p class="dark">
  ...
  <div id="order-detail-content" class="table_block table-responsive">
    <table class="table table-bordered">
      <thead>
        <tr>
          {if $return_allowed}<th class="first_item input_without_label"><input type="checkbox" /></th>{/if}
      ...
      {foreach from=$products item=product name=products}
        {if !isset($product.deleted)}
            ...
            <tr class="item">
              ...
              <input class="order_qte_input form-control grey" name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" />
                <div class="clearfix return_quantity_buttons">
                  <a href="#" class="return_quantity_down btn btn-secondary-2 button-minus"><span><i class="fa fa-minus"></i></span></a>
                  <a href="#" class="return_quantity_up btn btn-secondary-2 button-plus"><span><i class="fa fa-plus"></i></span></a>
                </div>
            ...
            {foreach $product.customizedDatas as $customizationPerAddress}
              {foreach $customizationPerAddress as $customizationId => $customization}
                <tr class="alternate_item">
                  {if $return_allowed}<td class="order_cb input_without_label"><input type="checkbox" id="cb_{$product.id_order_detail|intval}" name="customization_ids[{$product.id_order_detail|intval}][]" value="{$customizationId|intval}" /></td>{/if}
                  <td colspan="2">
          ...
          {if $product.product_quantity > $product.customizationQuantityTotal}
            <tr class="item">
              {if $return_allowed}<td class="order_cb input_without_label"><input type="checkbox" id="cb_{$product.id_order_detail|intval}" name="ids_order_detail[{$product.id_order_detail|intval}]" value="{$product.id_order_detail|intval}" /></td>{/if}
              <td><label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'html':'UTF-8'}{else}--{/if}</label></td>
              <td class="bold">
  ...
  {if $return_allowed}
    <div id="returnOrderMessage">
      ...
      <p class="form-group">
        <button type="submit" name="submitReturnMerchandise" class="btn btn-secondary btn-sm icon-right"><span>{l s='Make an RMA slip'}</span></button>
  ...
  {if !$is_guest}
    ...
    <form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
      ...
      <div class="submit">
        ...
        <button type="submit" name="submitMessage" class="btn btn-secondary btn-sm icon-right"><span>{l s='Send'}</span></button>
      </div>
    </form>
  {else}
