...
{if $orders}
          ...
          {if $page != 1}
            ...
            <li id="pagination_previous" class="pagination_previous">
              <a href="{summarypaginationlink p=$p_previous n=$nbpagination}" title="{l s='Previous' mod='loyalty'}" rel="prev">
                <i class="fa fa-chevron-left"></i> <b>{l s='Previous' mod='loyalty'}</b>
              </a>
            </li>
          {else}
          ...
          {if $orders|@count > $page * $nbpagination}
            ...
            <li id="pagination_next" class="pagination_next">
              <a href="{summarypaginationlink p=$p_next n=$nbpagination}" rel="next">
                <b>{l s='Next' mod='loyalty'}</b> <i class="fa fa-chevron-right"></i>
              </a>
            </li>
          {else}
  ...
  <p>{l s='Vouchers generated here are usable in the following categories : ' mod='loyalty'}{if $categories}{$categories}{else}{l s='All' mod='loyalty'}{/if}</p>
  {if $transformation_allowed}
    <p class="text-center">
      <a class="btn btn-default" href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'], true)|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure you want to transform your points into vouchers?' mod='loyalty' js=1}');">{l s='Transform my points into a voucher of' mod='loyalty'}
        ...
        {foreach from=$discounts item=discount name=myLoop}
          <tr class="alternate_item">
            ...
            <td class="history_method">
              <a rel="#order_tip_{$discount->id|intval}" class="cluetip"
                 title="{l s='Generated by these following orders' mod='loyalty'}"
                 href="{$smarty.server.SCRIPT_NAME|escape:'html':'UTF-8'}">{l s='more...' mod='loyalty'}</a>
...
<ul class="footer_links clearfix">
  <li>
    <a class="btn btn-secondary btn-sm" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='Back to Your Account' mod='loyalty'}" rel="nofollow">
      <span>
  ...
  <li>
    <a class="btn btn-secondary btn-sm" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{l s='Home' mod='loyalty'}">
      <span>
        <i class="fa fa-chevron-left"></i>
