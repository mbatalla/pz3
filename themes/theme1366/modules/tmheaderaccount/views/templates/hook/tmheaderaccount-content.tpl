...
<ul class="header-login-content toogle_content{if $is_logged} is-logged{/if}">
  ...
  <li {if $hook != 'left' && $hook != 'right'}class="{$configs.TMHEADERACCOUNT_DISPLAY_STYLE|escape:'quotes':'UTF-8'}"{/if}>
    ...
    <p class="logout">
      <a class="btn btn-primary btn-md" href="{$link->getPageLink('index')|escape:'html':'UTF-8'}?mylogout" title="{l s='Sign out' mod='tmheaderaccount'}" rel="nofollow">
  ...
  <li class="login-content">
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post">
      ...
      <div class="form_content clearfix">
        ...
        <p>
          <a href="{$link->getPageLink('my-account', true)|escape:'htmlall':'UTF-8'}" class="create">
    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" class="std">
      ...
      <div class="account_creation">
        <div class="clearfix">
          ...
          {foreach from=$genders key=k item=gender}
            <div class="radio-inline">
              ...
              <label for="id-gender{$gender->id}" class="top">{$gender->name}</label>
            </div>
          {/foreach}
        </div>
        ...
        <div class="required dni form-group">
          ...
          <input type="text" class="text form-control validate" name="dni" id="dni" data-validate="isDniLite" value="" />
          <span class="form_info">Sin puntos ni guiones</span>
        </div>
        <div class="required form-group">
          <input type="email" class="is_required validate form-control" data-validate="isEmail" name="email" id="email-create" placeholder="{l s='Email' mod='tmheaderaccount'}" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}"/>
        </div>
        <div class="required password form-group">
          <input type="password" class="is_required validate form-control" data-validate="isPasswd" name="passwd" id="passwd-create" placeholder="{l s='Password' mod='tmheaderaccount'}"/>
      ...
      {if $b2b_enable}
        <div class="account_creation">
          ...
          <p class="form-group">
            ...
            <input type="text" class="form-control" name="website" id="website-tmha" value="{if isset($smarty.post.website)}{$smarty.post.website}{/if}"/>
          </p>
        </div>
      {/if}
          ...
          {foreach from=$dlv_all_fields item=field_name}
            ...
            {elseif $field_name eq "vat_number"}
              <div style="display:none;">
                <p class="form-group">
                  <label for="vat_number-tmha">{l s='VAT number' mod='tmheaderaccount'}{if in_array($field_name, $required_fields)}
                      <sup>*</sup>
                    {/if}</label>
    ...
    <form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std">
      <fieldset>
        <div class="form-group">
          ...
          <div class="alert alert-danger" style="display:none;"></div>
          <input class="form-control" type="email" name="email" id="email-forgot" placeholder="{l s='Email address' mod='tmheaderaccount'}" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'|stripslashes}{/if}"/>
        </div>
        <p class="submit">
