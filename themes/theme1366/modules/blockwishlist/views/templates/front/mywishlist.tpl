<div id="mywishlist">
    ...
    <form method="post" class="std box" id="form_wishlist">
      <fieldset>
        ...
        <div class="form-group">
          ...
          <input type="text" id="name" name="name" class="inputTxt form-control" value="{if isset($smarty.post.name) && $errors|@count > 0}{$smarty.post.name|escape:'html':'UTF-8'}{/if}" />
        </div>
        <p class="submit">
          <button id="submitWishlist" class="btn btn-default btn-md icon-right" type="submit" name="submitWishlist">
        ...
        <table class="table table-bordered">
          <thead>
            <tr>
              ...
              <th class="item mywishlist_first td_none">{l s='Viewed' mod='blockwishlist'}</th>
              <th class="item mywishlist_second td_none">{l s='Created' mod='blockwishlist'}</th>
          ...
          <tbody>
            {section name=i loop=$wishlists}
              <tr id="wishlist_{$wishlists[i].id_wishlist|intval}">
                ...
                <td class="td_none">{$wishlists[i].counter|intval}</td>
                <td class="td_none">{$wishlists[i].date_add|date_format:"%Y-%m-%d"}</td>
                <td>
  ...
  <ul class="footer_links clearfix">
    <li>
      <a class="btn btn-secondary btn-sm icon-left" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='Back to Your Account' mod='blockwishlist'}">
        <span>
    ...
    <li>
      <a class="btn btn-secondary btn-sm icon-left" href="{$base_dir|escape:'html':'UTF-8'}" title="{l s='Home' mod='blockwishlist'}">
        <span>
