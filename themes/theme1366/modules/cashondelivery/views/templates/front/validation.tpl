...
<form action="{$link->getModuleLink('cashondelivery', 'validation', [], true)|escape:'html'}" method="post">
  ...
  <p class="cart_navigation" id="cart_navigation">
    <a href="{$link->getPageLink('order', true)}?step=3" class="btn btn-secondary"><i class="fa fa-chevron-left"></i>{l s='Other payment methods' mod='cashondelivery'}</a>
    <button type="submit" class="btn btn-secondary btn-md"><span>{l s='I confirm my order' mod='cashondelivery'}</span></button>
  </p>
</form>