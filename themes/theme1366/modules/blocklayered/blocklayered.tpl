{if $nbr_filterBlocks != 0}
  <section id="layered_block_left" class="block">
    <span id="filter-toggle"><span>{l s='Filter' mod='blocklayered'}</span></span>
        ...
        <div>
                        ...
                        {if isset($filter.slider)}
                          {if $smarty.foreach.f_values.first}
                            <li>
                              ...
                              {if $filter.format == 1}
                                {l s='%1$s - %2$s'|sprintf:{displayPrice price=$filter.values[0]}:{displayPrice price=$filter.values[1]}|escape:'html':'UTF-8' mod='blocklayered'}
                              {else}
                                {l s='%1$s %3$s - %2$s %3$s'|sprintf:$filter.values[0]:$filter.values[1]:$filter.unit|escape:'html':'UTF-8' mod='blocklayered'}
                              {/if}
                            </li>
                          {/if}
                        {else}
          ...
          {foreach from=$filters item=filter}
            {if isset($filter.values)}
              {if isset($filter.slider)}
                <div class="layered_{$filter.type} clearfix {if $filter.type == 'price' && $PS_CATALOG_MODE}hidden{/if}" style="display: none;">
              {else}
                <div class="layered_filter {if $filter.type == 'price' && $PS_CATALOG_MODE}hidden{/if}">
              {/if}
                          ...
                          {/if}
                            <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                              {if !$value.nbr}
                                <span>{$value.name|escape:'html':'UTF-8'}</span>
                              {else}
                                <a href="{$value.link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}><span>{$value.name|escape:'html':'UTF-8'}</span></a>
                              {/if}
                            </label>
                            {if $layered_show_qties}<span>{$value.nbr}</span>{/if}
                          </li>
                        {/if}
                      {/foreach}
                    {else}
                ...
                {else}
                  {if $filter.filter_type == 0}
                    <div class="layered_slider_container">
                      ...
                      <span id="layered_{$filter.type}_range_from"></span>
                      <span id="layered_{$filter.type}_range_to"></span>
                    </div>
                  {else}
                    {if $filter.filter_type == 1}
                      <li class="nomargin">
                        <div class="first-item form-group">
                          <label class="form-label" for="layered_{$filter.type}_range_min">{l s='From' mod='blocklayered'}({$filter.unit})</label>
                        ...
                        <div class="form-group">
                          <label class="form-label" for="layered_{$filter.type}_range_max">{l s='to' mod='blocklayered'}({$filter.unit})</label>
