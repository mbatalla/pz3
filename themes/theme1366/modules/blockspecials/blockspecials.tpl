...
<section id="special_block_right" class="block">
  ...
  <div class="block_content products-block">
    {if $special}
          ...
          <div class="product-content">
            ...
            {if isset($special.description_short) && $special.description_short}
              <p class="product-description">
                {$special.description_short|strip_tags:'UTF-8'|truncate:57}
              </p>
            {/if}
            <div class="price-box">
              {if !$PS_CATALOG_MODE}
                ...
                {if $special.specific_prices}
                  ...
                  {if $specific_prices.reduction_type == 'percentage' && ($specific_prices.from == $specific_prices.to || ($smarty.now|date_format:'%Y-%m-%d %H:%M:%S' <= $specific_prices.to && $smarty.now|date_format:'%Y-%m-%d %H:%M:%S' >= $specific_prices.from))}
      ...
      <div>
        <a class="btn btn-secondary-2 btn-sm" href="{$link->getPageLink('prices-drop')|escape:'html':'UTF-8'}" title="{l s='All specials' mod='blockspecials'}">
          <span>
