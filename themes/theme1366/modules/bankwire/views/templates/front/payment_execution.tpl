{capture name=path}
  <a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" title="{l s='Go back to the Checkout' mod='bankwire'}">
    {l s='Checkout' mod='bankwire'}
  </a>
  ...
  <form action="{$link->getModuleLink('bankwire', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
    <div class="box cheque-box">
      ...
      <p>- {l s='Bank wire account information will be displayed on the next page.' mod='bankwire'}<br />
        - {l s='Please confirm your order by clicking "I confirm my order".' mod='bankwire'}</p>
    </div><!-- .cheque-box -->

    <p class="cart_navigation clearfix" id="cart_navigation">
      <a class="btn btn-secondary btn-md icon-left" href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" title="{l s='Other payment methods' mod='bankwire'}">{l s='Other payment methods' mod='bankwire'} </a>
      <button class="btn btn-secondary btn-md icon-right" type="submit"><span>{l s='I confirm my order' mod='bankwire'}</span></button>
    </p>
  </form>
{/if}