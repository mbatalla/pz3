...
{else}
  <form action="{$link->getModuleLink('cheque', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
    ...
    <p class="cart_navigation clearfix" id="cart_navigation">
      <a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" class="btn btn-secondary btn-md icon-left" title="{l s='Other payment methods' mod='cheque'}">
        {l s='Other payment methods' mod='cheque'}
      </a>
      <button type="submit" class="btn btn-secondary btn-md icon-right">
        <span>
