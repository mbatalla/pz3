  ...
  {else}
    ...
    <form action="{$link->getPageLink('identity', true)|escape:'html':'UTF-8'}" method="post" class="std">
      <fieldset>
        ...
        {/if}
        <div class="form-group">
          <button type="submit" name="submitIdentity" class="btn btn-secondary btn-sm">
            <span>{l s='Save'}</span>
          </button>
        </div>
      </fieldset>
...
<ul class="footer_links clearfix">
  <li>
    <a class="btn btn-secondary btn-sm icon-left" href="{$link->getPageLink('my-account', true)}" title="{l s='Back to your account'}">
      <span>
