...
function changeAddressDelivery(obj) {
  ...
  if (new_id_address_delivery > 0) { // Change the delivery address
    ...
    // Add a new line for this product
  ...
  for (i in product_list) {
    ...
    if (reduction && typeof(initial_price) !== 'undefined') {
      ...
      if (initial_price !== '' && product_list[i].price_without_quantity_discount > product_list[i].price) {
        initial_price_text = '<span class="price-percent-reduction small">&nbsp;' + price_reduction + reduction_symbol + '&nbsp;</span><span class="old-price">' + initial_price + '</span>';
