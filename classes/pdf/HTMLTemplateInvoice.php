...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    public function getContent()
    {
        ...
        $sql = "SELECT Tbk_codigo_autorizacion, Tbk_numero_final_tarjeta, Tbk_tipo_pago, Tbk_numero_cuotas, Tbk_monto FROM webpay where Tbk_orden_compra = '". (int)$this->order->id_cart ."';";
        $row = Db::getInstance()->getRow($sql);

        $id_qad = (int)$this->order->id_cart;    
        $tarjeta = $row['Tbk_numero_final_tarjeta'];
        $cod_autorizacion = $row['Tbk_codigo_autorizacion'];
        $tipo_pago = $row['Tbk_tipo_pago'];
        $cuotas = $row['Tbk_numero_cuotas'];
        $monto = $row['Tbk_monto'];
