...
add_action( 'init', 'warsaw_bunch_theme_init');


if( !function_exists( 'warsaw_set' ) ) {
	function warsaw_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('warsaw_printr' ) ) {
	function warsaw_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('warsaw__font_awesome' ) ) {
	function warsaw_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = warsaw_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('warsaw_load_class' ) ) {
	function warsaw_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
warsaw_load_class( 'enqueue', 'helpers', false );

warsaw_load_class( 'bootstrap_walker', 'helpers', false );


if( warsaw_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function warsaw_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include WARSAW_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include WARSAW_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = WARSAW_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = WARSAW_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( warsaw_set( $_GET, 'page' ) == 'warsaw'.'_option' ) 
		{
			if( warsaw_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'warsaw_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = WARSAW_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'warsaw'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'warsaw'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'warsaw' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'warsaw' ), // menu label
	));
