...
class Bunch_Backup_class
{
	...
	function theme_options_import($file = '')
	{
		...
		$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."terms WHERE ".$wpdb->prefix."terms.slug = 'Main-Menu' OR ".$wpdb->prefix."terms.slug = 'Main-Menu-Left' OR ".$wpdb->prefix."terms.slug = 'Main-Menu-Right'");
		if($res){
			$nav_menu = array('');
			$nav_menu['nav_menu_locations']['main_menu'] = $res[0]->term_id;
			$nav_menu['nav_menu_locations']['main_menu_left'] = $res[1]->term_id;
			$nav_menu['nav_menu_locations']['main_menu_right'] = $res[2]->term_id;
