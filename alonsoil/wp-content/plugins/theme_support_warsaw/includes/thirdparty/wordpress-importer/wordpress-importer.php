...
<?php
	...
	function process_attachment( $post, $url ) {
		...
		if( !strstr($url, 'http://') && !strstr($url, 'https://') ) $url = rtrim( $this->base_url, '/' ). '/' . $url;
		//echo rtrim( $this->base_url, '/' ). '/' . $url;exit;
		//echo $url.'<br>';exit;
	...
	function fetch_remote_file( $url, $post ) {
		...
		$url = get_template_directory_uri().'/images/placeholder.jpg';
		...
		$http_object = new WP_Http;
		$headers = $http_object->get( $url );
		...
		if( !is_wp_error($headers) && is_object($wp_filesystem) ) {
		...
		if ( $headers['response']['code'] != '200' ) {
			...
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'wordpress-importer'), $headers['response']['code'], get_status_header_desc($headers['response']['code']) ) );
		}
		//echo $upload['file'];exit;
